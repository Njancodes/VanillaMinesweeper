class TileComponent extends HTMLElement {
    ...
    update() {
        if (this.querySelector("img")) {
            ...
            if (!this.show) {
                if (!this.flagged) {
                    tileImg.src = `VanillaMinesweeper/assets/h-tile.png`
                }else{
                    tileImg.src = `VanillaMinesweeper/assets/flag-tile.png`
                }
            } else {
                tileImg.src = `VanillaMinesweeper/assets/${this.value}-tile.png`
            }
        }
    }
